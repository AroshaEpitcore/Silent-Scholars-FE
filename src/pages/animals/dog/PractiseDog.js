...
import { FaDog, FaArrowLeft, FaPlay, FaCheckCircle } from "react-icons/fa";
    ...
    const saveScore = async () => {
        ...
        try {
            ...
            await GuardianDataService.recordActivity({
                activityName: "Completed Animal Signs Lesson - Dog",
        ...
        if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
            ...
            if (totalLandmarks.length === 63 && !finishRef.current) {
                try {
                    ...
                    const currentPoseName = DogPoses[currentPoseRef.current]?.name;
                    if (prob >= 0.9 && predicted === currentPoseName) {
                        ...
                        if (elapsed >= HOLD_DURATION) {
                            ...
                            setHoldProgress(0);
                            setHoldSeconds(0);
                            if (currentPoseRef.current >= DogPoses.length - 1) {
        ...
        <div className="an-page">
            <div className="an-container">
                ...
                <div className="an-breadcrumb">
                    ...
                    <Link to="/learn-dog">{t("learnDog")}</Link>
                    <span className="an-breadcrumb-sep">›</span>
                ...
                <div className="traffic-signs-grid">
                    <div className="ts-card">
                        <div className="ts-card-header"><FaDog /> {t("referencePose")}</div>
                        ...
                        <div className="card-body">
                            ...
                            {isDone && (
                                <div className="an-status-panel">
                                    ...
                                    <button className="an-btn-back" onClick={() => navigate("/learn-dog")}>{t("backToLearn")}</button>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="an-actions">
                    <button className="an-btn-outline" onClick={() => navigate("/learn-dog")}><FaArrowLeft /> {t("backToLearn")}</button>
                </div>
            </div>
        </div>
    );
