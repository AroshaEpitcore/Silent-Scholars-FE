...
import { FaCat, FaArrowLeft, FaPlay, FaCheckCircle } from "react-icons/fa";
import { auth } from "../../../firebase";
import "../animals.css";
import "../../traffic/traffic-signs.css";

const HOLD_DURATION = 30; // seconds
const DONE_STEP = 3;      // 2 poses + done
...
export default function PractiseCat() {
    ...
    // Refs so async onResults always reads latest values
    ...
    const saveScore = async () => {
        ...
        try {
            ...
            await GuardianDataService.recordActivity({
                activityName: "Completed Animal Signs Lesson - Cat",
        ...
        if (results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
            ...
            if (totalLandmarks.length === 63 && !finishRef.current) {
                try {
                    ...
                    const currentPoseName = CatPoses[currentPoseRef.current]?.name;
                    if (prob >= 0.9 && predicted === currentPoseName) {
                        ...
                        if (elapsed >= HOLD_DURATION) {
                            ...
                            setHoldProgress(0);
                            setHoldSeconds(0);
                            if (currentPoseRef.current >= CatPoses.length - 1) {
                                ...
                                currentPoseRef.current += 1;
                                currentStepRef.current += 1;
                                setCurrentPose(currentPoseRef.current);
        ...
        <div className="an-page">
            <div className="an-container">
                ...
                <div className="an-breadcrumb">
                    ...
                    <Link to="/learn-cat">{t("learnCat")}</Link>
                    <span className="an-breadcrumb-sep">›</span>
                ...
                <div className="traffic-signs-grid">
                    <div className="ts-card">
                        <div className="ts-card-header"><FaCat /> {t("referencePose")}</div>
                        ...
                        <div className="card-body">
                            ...
                            {isDone && (
                                <div className="an-status-panel">
                                    ...
                                    <button className="an-btn-back" onClick={() => navigate("/learn-cat")}>{t("backToLearn")}</button>
                                </div>
                            )}
                        </div>
                    </div>
                ...
                <div className="an-actions">
                    <button className="an-btn-outline" onClick={() => navigate("/learn-cat")}><FaArrowLeft /> {t("backToLearn")}</button>
                </div>
            </div>
        </div>
    );
